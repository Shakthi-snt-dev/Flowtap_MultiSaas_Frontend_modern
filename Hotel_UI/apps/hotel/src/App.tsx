...
import { BedDouble, CalendarDays, LayoutList } from 'lucide-react'
...
const OnboardingPage        = React.lazy(() => import('@flowtap/pages-core').then((m) => ({ default: m.HotelOnboardingPage })))
...
const RoomTypesPage = React.lazy(() => import('./pages/RoomTypesPage').then((m) => ({ default: m.RoomTypesPage })))

const hotelNav = [
  { label: 'Bookings',   href: '/bookings',   icon: <CalendarDays className="w-5 h-5" /> },
  { label: 'Rooms',      href: '/rooms',      icon: <BedDouble    className="w-5 h-5" /> },
  { label: 'Room Types', href: '/room-types', icon: <LayoutList   className="w-5 h-5" /> },
]
    ...
    <React.Suspense fallback={<PageLoader />}>
      <Routes>
        ...
        <Route element={<ProtectedRoute requireTenant />}>
          <Route element={<AppLayout extraNavItems={hotelNav} />}>
